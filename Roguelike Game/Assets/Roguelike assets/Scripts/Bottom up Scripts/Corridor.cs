...
public class Corridor
{
	...
	public void SetupCorridor (Room room, IntRange length, IntRange roomWidth, IntRange roomHeight, int columns, int rows, bool firstCorridor)
	{
		...
		if (!firstCorridor && direction == oppositeDirection)
		{
			...
			// his means instead of rotating 180 (the opposite direction) we're rotating 90
		...
		//Complex ver of an if statement (more than one if basically)
		switch (direction)
		{
		...
		//Clamp the length of the corridor to make sure it doesn't go off the board
